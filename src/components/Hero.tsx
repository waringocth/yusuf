    ...
    <section className="relative min-h-screen flex flex-col justify-end pb-32 sm:pb-24 overflow-visible">
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pt-28 sm:pt-0">
        ...
        >
          ...
          <motion.div
            variants={itemVariants}
            className="flex flex-row flex-wrap items-center justify-center sm:justify-start gap-2 sm:gap-4 mb-8"
          >
            {trustBadges.map((badge) => (
              <div key={badge.label} className="flex items-center gap-1.5 bg-white/10 backdrop-blur-sm border border-white/15 rounded-full px-3 py-1.5 sm:px-4 sm:py-2">
                <badge.icon className={`w-3 h-3 sm:w-4 sm:h-4 ${badge.color}`} />
                <span className="text-white/90 text-[10px] sm:text-sm font-medium whitespace-nowrap">{badge.label}</span>
              </div>
            ))}
          </motion.div>
